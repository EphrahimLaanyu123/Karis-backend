...
// GET /api/events - Get all events (including ticket counts)
router.get('/', async (req, res) => {
  try {
    const events = await Event.find().sort({ date: -1 }); // Sorted descending by date
    res.status(200).json(events); // Each event has totalTickets and remainingTickets
  } catch (err) {
